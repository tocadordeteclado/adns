/**
 * adh-opts.c - tabelas úteis de manipulação de opções de programa
 *     de cliente resolvedor de uso geral, etc.
 *
...
static const struct optioninfo global_options[] = {
    ...
    { ot_flag,     "Do not look at environment variables at all",                "e",  "env",         &ov_env,    0                                                },
    { ot_flag,     "Read queries on stdin instead of using args",                "f",  "pipe",        &ov_pipe,   1                                                },
    { ot_flag,     "Allow answers to be reordered",                              "a",  "asynch",      &ov_asynch, 1                                                },
    { ot_desconly, "answer/error output format and destination (see below):"                                                                                       },
    { ot_value,    "Answers to stdout, errors as messages to stderr (default)",  "Fs", "fmt-simple",  &ov_format, fmt_simple                                       },
    { ot_value,    "Answers and errors both to stdout in parseable format",      "Fi", "fmt-inline",  &ov_format, fmt_inline                                       },
    { ot_value,    "Fully-parseable output format (default for --asynch)",       "Fa", "fmt-asynch",  &ov_format, fmt_asynch                                       },
    { ot_desconly, "global verbosity level:"                                                                                                                       },
    { ot_value,    "Do not print anything to stderr",                            "Vq", "quiet",       &ov_verbose, adns_if_noerrprint                              },
    { ot_value,    "Report unexpected kinds of problem only  (default)",         "Vn", "no-quiet",    &ov_verbose, 0                                               },
    { ot_value,    "Debugging mode",                                             "Vd", "debug",       &ov_verbose, adns_if_debug                                   },
    { ot_value,    "Do not do for-developer consistency checks",                   0,  "no-checkc",   &ov_checkc,  0                                               },
    { ot_value,    "Do for-developer consistency checks",                          0,  "checkc",      &ov_checkc,  adns_if_checkc_freq                             },
    { ot_value,    "Do for-developer consistency checks very often",               0,  "checkc-freq", &ov_checkc,  adns_if_checkc_freq                             },
    { ot_desconly, "other global options:"                                                                                                                         },
    { ot_funcarg,  "Configuration to use instead of /etc/resolv.conf",             0,  "config",      0,           0,                   of_config, "<config-text>" },
    { ot_func,     "Print version number",                                         0,  "version",     0,           0,                   of_version                 },
    { ot_func,     "Print usage information",                                      0,  "help",        0,           0,                   of_help                    },
    { ot_end }
};
...
static const struct optioninfo perquery_options[] = {
    ...
    { ot_funcarg,  "Query type (see below)",                                           "t", "type",               0,                    0, &of_type,    "type"         },
    { ot_funcarg,  "Do reverse query (address -> name lookup)",                        "i", "ptr",                0,                    0, &of_ptr,     "addr"         },
    { ot_funcarg2, "Lookup in in-addr-like `zone' (eg MAPS RBL)",                        0, "reverse",            0,                    0, &of_reverse, "addr", "zone" },
    { ot_desconly, "per-query binary options:"                                                                                                                         },
    { ot_flag,     "Use the search list",                                              "s", "search",             &ov_search,           1                              },
    { ot_flag,     "Let query domains contain quote-requiring chars",                 "Qq", "qc-query",           &ov_qc_query,         1                              },
    { ot_flag,     "Let hostnames in answers contain ...",                            "Qa", "qc-anshost",         &ov_qc_anshost,       1                              },
    { ot_flag,     "Prevent CNAME target domains from containing ...",                "Qc", "qc-cname",           &ov_qc_cname,         0                              },
    { ot_flag,     "Force use of a virtual circuit",                                   "u", "tcp",                &ov_tcp,              1                              },
    { ot_flag,     "Do not display owner name in output",                             "Do", "show-owner",         &ov_pqfr.show_owner,  0                              },
    { ot_flag,     "Do not display RR type in output",                                "Dt", "show-type",          &ov_pqfr.show_type,   0                              },
    { ot_flag,     "Do not display CNAME target in output",                           "Dc", "show-cname",         &ov_pqfr.show_cname,  0                              },
    { ot_desconly, "per-query TTL mode (NB TTL is minimum across all info in reply):"                                                                                  },
    { ot_value,    "Show the TTL as a TTL",                                           "Tt", "ttl-ttl",            &ov_pqfr.ttl,         tm_rel                         },
    { ot_value,    "Show the TTL as a time_t when the data might expire",             "Ta", "ttl-abs",            &ov_pqfr.ttl,         tm_abs                         },
    { ot_value,    "Do not show the TTL (default)",                                   "Tn", "no-ttl",             &ov_pqfr.ttl,         tm_none                        },
    { ot_desconly, "per-query address family options (for `addr' etc.):"                                                                                               },
    { ot_value,    "Only return IPv4 addresses",                                      "A4", "--addr-ipv4-only",   &ov_afflags,          adns_qf_want_ipv4              },
    { ot_value,    "Only return IPv6 addresses",                                      "A6", "--addr-ipv6-only",   &ov_afflags,          adns_qf_want_ipv6              },
    { ot_value,    "Return all available addresses (default)",                        "Aa", "--addr-all",         &ov_afflags,          0                              },
    { ot_flag,     "Return IPv4 addresses as IPv6-mapped",                            "Am", "--addr-ipv6-mapped", &ov_v6map,            1                              },
    { ot_desconly, "per-query CNAME handling mode:"                                                                                                                    },
    { ot_value,    "Call it an error if a CNAME is found",                            "Cf", "cname-reject",       &ov_cname,            adns_qf_cname_forbid           },
    { ot_value,    "Allow references to CNAMEs in other RRs",                         "Cl", "cname-loose",        &ov_cname,            adns_qf_cname_loose            },
    { ot_value,    "CNAME ok for query domain, but not in RRs (default)",             "Cs", "cname-ok",           &ov_cname,            0                              },
    { ot_desconly, "asynchronous/pipe mode options:"                                                                                                                   },
    { ot_funcarg,  "Set <id>, default is decimal sequence starting 0",                   0, "asynch-id",          0,                    0, &of_asynch_id, "id"         },
    { ot_funcarg,  "Cancel the query with id <id> (no error if not found)",              0, "cancel-id",          0,                    0, &of_cancel_id, "id"         },
    { ot_end }
};
            ...
            switch (oip->type)
            {
                case ot_flag:
                    if (!oip->value)
                    {
                        if (oip->sopt)
                        {
                            printf(" +%-*s --no-%-*s %s\n", maxsopt, oip->sopt, maxlopt - 2, oip->lopt, oip->desc);
                        } else
                        {
                            printf(" --no-%-*s %s\n", maxlopt + maxsopt + 1, oip->lopt, oip->desc);
                        }
                ...
                case ot_func:
                    if (oip->sopt)
                    {
                        printf(" -%-*s --%-*s %s\n", maxsopt, oip->sopt, maxlopt + 1, oip->lopt, oip->desc);
                    } else
                    {
                        printf(" --%-*s %s\n", maxlopt + maxsopt + 3, oip->lopt, oip->desc);
                    }
                ...
                case ot_funcarg:
                    if (oip->sopt)
                    {
                        l = (maxlopt + maxsopt - 9 - (strlen(oip->sopt) + strlen(oip->lopt) + 2 * strlen(oip->argdesc)));
                        printf(" -%s<%s> / --%s <%s>%*s%s\n", oip->sopt, oip->argdesc, oip->lopt, oip->argdesc, l > 2 ? l : 2, "", oip->desc);
                    } else
                    {
                        l = (maxlopt + maxsopt + 1 - (strlen(oip->lopt) + strlen(oip->argdesc)));
                        printf(" --%s <%s>%*s%s\n", oip->lopt, oip->argdesc, l > 2 ? l : 2, "", oip->desc);
                    }
                ...
                case ot_funcarg2:
                    ...
                    l = (maxlopt + maxsopt - 2 - (strlen(oip->lopt) + strlen(oip->argdesc) + strlen(oip->argdesc2)));
                    printf(" --%s <%s> <%s>%*s%s\n", oip->lopt, oip->argdesc, oip->argdesc2, l > 2 ? l : 2, "", oip->desc);
...
static const struct optioninfo *find1(const char **optp, const struct optioninfo *table, comparer_type *comparer)
{
    for (;;)
    {
        if (table->type == ot_end)
        {
            return 0;
        }

        if (comparer(optp, table))
        {
            return table;
        }
