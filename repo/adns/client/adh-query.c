/**
 * adh-query.c - útil programa cliente resolvedor de uso geral faz
 *     consultas e imprime respostas.
 *
...
static void prep_query(struct query_node **qun_r, int *quflags_r)
{
    ...
    *quflags_r = (ov_search ? adns_qf_search : 0) |
...
static void sockaddr_aton(const char *text, adns_rr_addr *a)
{
    ...
    a->len = sizeof(a->addr);
    err = adns_text2addr(text, 0, adns_qf_addrlit_scope_forbid, &a->addr.sa, &a->len);

    if (err == EINVAL)
    {
        usageerr("invalid IP address %s", text);
    } else if (err)
    {
        sysfail("adns_text2addr", err);
    }
}
...
void of_ptr(const struct optioninfo *oi, const char *arg, const char *arg2)
{
    ...
    r = adns_submit_reverse(ads, &a.addr.sa, ov_type == adns_r_none ? adns_r_ptr : ov_type, quflags, qun, &qun->qu);
...
static void print_ttl(struct query_node *qun, adns_answer *answer)
{
    ...
    switch (qun->pqfr.ttl)
    {
        ...
        case tm_rel:
            ...
            {
                sysfail("get current time", errno);
            }
...
static void print_owner_ttl(struct query_node *qun, adns_answer *answer)
{
    ...
    {
        print_withspace(owner_show(qun,answer));
    }
...
static void check_status(adns_status st)
{
    ...
    for (minrcode=0, spp = statuspoints; spp < statuspoints + (sizeof(statuspoints) / sizeof(statuspoints[0])); spp++)
    {
        if (st > *spp)
        {
            minrcode++;
...
static void print_status(adns_status st, struct query_node *qun, adns_answer *answer)
{
    ...
    if (printf("%s %d %s ", statustypeabbrev, st, statusabbrev) == EOF)
    {
        outerr();
    }
...
static void print_dnsfail(adns_status st, struct query_node *qun, adns_answer *answer)
{
    ...
    if (r == EOF)
    {
        sysfail("write error message to stderr", errno);
    }
}
...
void query_done(struct query_node *qun, adns_answer *answer)
{
    ...
    if (nrrs)
    {
        for (rrn = 0, rrp = answer->rrs.untyped; rrn < nrrs; rrn++, rrp += answer->rrsz)
        {
            if (realowner)
            {
...
void of_cancel_id(const struct optioninfo *oi, const char *arg, const char *arg2)
{
    ...
    for (qun = outstanding.head; qun && strcmp(qun->id, arg); qun = qun->next)
    {
    }
