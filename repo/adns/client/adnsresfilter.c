/**
 * adnsresfilter.c - filtro que resolve, não faz parte da biblioteca.
 *
...
static void parseargs(const char *const *argv)
{
    ...
    while ((arg= *++argv))
    {
        if (arg[0] != '-')
        {
            usageerr("no non-option arguments are allowed");
        }
        ...
        if (arg[1] == '-')
        {
            ...
            {
                VERSION_PRINT_QUIT("adnsresfilter");
                quitnow(0);
            } else
            {
...
static void queueoutchar(int c)
{
    ...
    if (!entry || entry->addr || entry->textlen >= peroutqueuenode - (entry->textp - entry->buffer))
    {
        peroutqueuenode = !peroutqueuenode || !entry || entry->addr ? 128 : peroutqueuenode >= 1024 ? 4096 : peroutqueuenode << 2;
...
static void procaddr(void)
{
    ...
    if (foundthing == newthing)
    {
        ...
        r = adns_submit_reverse(ads, (const struct sockaddr*)&sa, rrt, 0, foundthing, &foundthing->qu);
...
static void readstdin(void)
{
    ...
    for (p = readbuf; r > 0; r--, p++)
    {
        ...
        } else if (cbyte == -1 && !bracket && !isalnum(c))
        {
            queueoutchar(c);
            startaddr();
        } else if (cbyte >= 0 && inbyte < 3 && c >= '0' && c <= '9' && (nbyte = bytes[cbyte] * 10 + (c - '0')) <= 255)
        {
            bytes[cbyte] = nbyte;
            addrtextbuf[inbuf++] = c;
            inbyte++;
...
static void startup(void)
{
    ...
    if (nonblock(0, 1))
    {
        sysfail("set stdin to nonblocking mode");
    }

    if (nonblock(1, 1))
    {
        sysfail("set stdout to nonblocking mode");
    }
...
int main(int argc, const char *const *argv)
{
    ...
    while (!inputeof || outqueue.head)
    {
        ...
        if ((entry = outqueue.head) && !outblocked)
        {
            ...
            adns_beforeselect(ads, &maxfd, &readfds, &writefds, &exceptfds, &tv, &tvbuf, &now);
        }
    ...
    if (nonblock(0, 0))
    {
        sysfail("un-nonblock stdin");
    }

    if (nonblock(1, 0))
    {
        sysfail("un-nonblock stdout");
    }
