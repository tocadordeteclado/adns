/**
 * adnstest.c - programa de teste simples, não faz parte da biblioteca.
 *
...
static void failure_errno(const char *what, int errnoval)
{
    switch (errnoval)
    {
        #define CE(e) \
            case e: \
                fprintf(stderr, "adns failure: %s: errno=" #e "\n", what); \
                break;
...
{
    fprintf(stdout, "%s(%s)%s%s", (!ri && rrtn) ? rrtn : "?", ri ? "?" : fmtn ? fmtn : "-", ri ? " " : "", ri ? adns_strerror(ri) : "");
}
...
int main(int argc, char *const *argv)
{
    ...
    if (!(argv[0] && argv[1]))
    {
        usageerr("no query domains supplied");
    }
    ...
    if (initstring)
    {
        r = adns_init_strcfg(&ads, (adns_if_debug | adns_if_noautosys | adns_if_checkc_freq) ^initflagsnum, stdout, initstring);
    } else
    {
        r = adns_init(&ads, (adns_if_debug | adns_if_noautosys) ^initflagsnum, 0);
    }

    if (r)
    {
        failure_errno("init", r);
    }
    ...
    for (qi = 0; qi < qc; qi++)
    {
        ...
        for (ti = 0; ti < tc; ti++)
        {
            ...
            fprintf(stdout,"%s flags %d type %d", domain, qflags, types[ti]&adns_rrt_reprmask);
    ...
    for (;;)
    {
        ...
        for (adns_forallqueries_begin(ads); (qu = adns_forallqueries_next(ads, &mcr));)
        {
            mc= mcr;
            assert(qu == mc->qu);
            assert(!mc->doneyet);
        ...
        fprintf(stdout, "%s%s: %s; nrrs=%d; cname=%s; owner=%s; ttl=%ld\n", ownflags[0] ? " ownflags=" : "", ownflags, strchr(ownflags,'a') ? adns_errabbrev(ans->status) : adns_strerror(ans->status), ans->nrrs, ans->cname ? ans->cname : "$", ans->owner ? ans->owner : "$", (long)ans->expires - (long)now.tv_sec);
