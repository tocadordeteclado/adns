...
/**
 * Principais funções de diagnóstico.
 */
...
void adns__vdiag(adns_state ads, const char *pfx, adns_initflags prevent, int serv, adns_query qu, const char *fmt, va_list al)
{
    ...
    if (!ads->logfn || (!(ads->iflags & adns_if_debug) && (!prevent || (ads->iflags & prevent))))
    {
        return;
    }
    ...
    {
        adns__vbuf_init(&vb);
        adns__lprintf(ads, "%sQNAME=%s, QTYPE=%s", bef, adns__diag_domain(qu->ads, -1, 0, &vb, qu->query_dgram, qu->query_dglen, DNS_HDRSIZE), qu->typei ? qu->typei->rrtname : "<unknown>");
    ...
    if (serv >= 0)
    {
        adns__lprintf(ads, "%sNS=%s", bef, adns__sockaddr_ntoa(&ads->servers[serv].addr.sa, buf));
...
/**
 * Funções adicionais de diagnóstico.
 */
...
const char *adns__diag_domain(adns_state ads, int serv, adns_query qu, vbuf *vb, const byte *dgram, int dglen, int cbyte)
{
    ...
    if (st)
    {
        ...
        if (!(adns__vbuf_appendstr(vb, "<bad format... ") && adns__vbuf_appendstr(vb, adns_strerror(st)) && adns__vbuf_appendstr(vb, ">") && adns__vbuf_append(vb, "", 1)))
        {
...
adns_status adns_rr_info(adns_rrtype type, const char **rrtname_r, const char **fmtname_r, int *len_r, const void *datap, char **data_r)
{
    ...
    if (len_r)
    {
        *len_r = typei->getrrsz(typei, type);
    }
...
void adns__isort(void *array, int nobjs, int sz, void *tempbuf, int (*needswap)(void *context, const void *a, const void *b), void *context)
{
    ...
    for (i = 0; i < nobjs; i++)
    {
        for (place = i; place > 0 && needswap(context, data + (place - 1)*sz, data + i*sz); place--);
