...
static adns_status mkquery_header(adns_state ads, vbuf *vb, int *id_r, int qdlen)
{
    ...
    *id_r = id = (ads->nextid++) & 0x0ffff;
    ...
    /**
     * QR=Q(0), OPCODE=QUERY(0000), !AA, !TC, RD.
     */
    MKQUERY_ADDB(0x01);

    /**
     * !RA, Z=000, RCODE=NOERROR(0000).
     */
    MKQUERY_ADDB(0x00);
...
static adns_status mkquery_footer(vbuf *vb, adns_rrtype type)
{
     ...
     * QTYPE.
     */
    MKQUERY_ADDW(type & adns_rrt_typemask);
 ...
 *
 */
static adns_status qdparselabel(adns_state ads, const char **p_io, const char *pe, char label_r[], int *ll_io, adns_queryflags flags)
{
    int ll;
    int c;
    const char *p;
...
void adns__querysend_tcp(adns_query qu, struct timeval now)
{
    ...
    /**
     * Redefina o tempo limite de inatividade.
     */
...
struct udpsocket *adns__udpsocket_by_af(adns_state ads, int af)
{
    ...
    {
        if (ads->udpsockets[i].af == af)
        {
            return &ads->udpsockets[i];
        }
    }
...
void adns__query_send(adns_query qu, struct timeval now)
{
    ...
    r = sendto(udp->fd,
        qu->query_dgram,
        qu->query_dglen, 0, &addr->addr.sa, addr->len);
