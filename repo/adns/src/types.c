/**
 * types.c
 *     - Código específico do tipo RR e o maquinário para chamá-lo.
 *
 ...
 * _in6addr            (pa, di, cs)
 * _addr               (pap, pa, di, div, csp, cs, gsz, qs, +search_sortlist_sa, dip_sockaddr, addr_rrtypes, addr_submit, icb_addr)
 * _domain             (pap, csp, cs)
 * _dom_raw            (pa)
 * _host_raw           (pa)
...
{
    /**
     * Nem len_r nem str_r podem ser nulos. O fim do datagrama
     * (overrun) é indicado retornando adns_s_invaliddata;
     */
...
static int search_sortlist(adns_state ads, int af, const void *ad)
{
    ...
    for (i = 0, slp = ads->sortlist; i < ads->nsortlist && !adns__addr_matches(af, ad, &slp->base, &slp->mask) && !(v6mappedp && adns__addr_matches(AF_INET, &a4, &slp->base, &slp->mask)); i++, slp++)
    {
    }
...
static adns_status csp_genaddr(vbuf *vb, int af, const void *p)
{
    ...
    err = adns_addr2text(&a.addr.sa, 0, buf, &len, 0);
    assert(!err);
 ...
 *
 */
static int di_in6addr(adns_state ads, const void *datap_a, const void *datap_b)
{
 ...
 *    +search_sortlist_sa, dip_sockaddr, addr_rrtypes, addr_submit, icb_addr)
 */
...
/**
 * Sobre o tratamento de CNAME em consultas addr.
 *
 * Uma consulta addr no nível do usuário é traduzida em várias consultas no
 * nível do protocolo e seu trabalho é remontar os resultados. Isso fica
 * complicado se as respostas não forem consistentes. Em particular, se
 * as respostas relatarem indireção inconsistente por meio de registros
 * CNAME (por exemplo, CNAMEs diferentes ou algumas indiretas por meio
 * de um CNAME e outras não), teremos falhas.
 *
 * Assim que recebermos uma resposta, mesmo que seja NODATA, definimos
 * adns__qf_addr_answer na consulta do processo principal. Isso nos
 * permitirá detectar um conflito entre uma resposta sem CNAME com
 * NODATA e um CNAME subsequente.
 *
 * Se detectarmos um conflito de qualquer tipo, pelo menos uma resposta
 * voltou com um registro CNAME, então escolhemos a primeira resposta
 * (um tanto arbitrariamente) como sendo o nome canônico 'correto' e
 * definimos isso na resposta da consulta do processo principal. plug
 * answer->cname. Descartamos os registros de endereço do nome com
 * falha. E, finalmente, cancelamos as consultas dos sub processos
 * pendentes e reenviamos as consultas de endereços para as famílias
 * de endereços que ainda não temos, com adns__qf_addr_cname definido
 * para que saibamos que estamos no estado de reparo.
 */

/**
 *
 */
static adns_status pap_addr(const parseinfo *pai, int in_rrty, size_t out_rrsz, int *cbyte_io, int cbyte_max, adns_rr_addr *out)
{
    int in_addrlen;
    int out_af;
    int out_salen;
    ...
    if ((*cbyte_io + in_addrlen) != cbyte_max)
    {
        return adns_s_invaliddata;
    }

    if (out_af == AF_INET && (pai->qu->flags & adns_qf_ipv6_mapv4) && (pai->qu->answer->type & adns__qtf_bigaddr))
    {
    ...
    {
        case AF_INET:
            out_salen = sizeof(out->addr.inet);
            break;

        case AF_INET6:
            out_salen = sizeof(out->addr.inet6);
            break;
...
{
    int err = pap_addr(pai,
        pai->qu->answer->type & adns_rrt_typemask,
        pai->qu->answer->rrsz,
        &cbyte,
        max,
        datap);
 ...
 *
 */
static int dip_sockaddr(adns_state ads, const struct sockaddr *sa, const struct sockaddr *sb)
{
    if (!ads->sortlist)
    {
        return 0;
...
static adns_status csp_addr(vbuf *vb, const adns_rr_addr *rrp)
{
    ...
    switch (rrp->addr.inet.sin_family)
    {
        ...
        a2t:
            err = adns_addr2text(&rrp->addr.sa, 0, buf, &len, 0);
            assert(!err);
...
{
    return type & adns__qtf_bigaddr ? sizeof(adns_rr_addr) : sizeof(adns_rr_addr_v4only);
}

/**
 *
 */
static unsigned addr_rrtypes(adns_state ads, adns_rrtype type, adns_queryflags qf)
{
    /**
     * Retorna uma máscara de sinalizadores addr_rf_... indicando
     * quais famílias de endereços são desejadas, dado um tipo de
     * consulta e sinalizadores.
     */
    ...
    } else
    {
        ...
        {
            qf |= (type & adns__qtf_manyaf) ? adns_qf_want_allaf : adns_qf_want_ipv4;
        }

        if (ads->iflags & adns_if_permit_ipv4)
        {
            permitaf |= adns_qf_want_ipv4;
        }

        if (ads->iflags & adns_if_permit_ipv6)
        {
            permitaf |= adns_qf_want_ipv6;
        }

        if (qf & permitaf)
        {
            qf &= permitaf | ~adns_qf_want_allaf;
        }
    }

    if (qf & adns_qf_want_ipv4)
    {
        want |= addr_rf_a;
    }

    if (qf & adns_qf_want_ipv6)
    {
        want |= addr_rf_aaaa;
    }
 ...
 *
 */
static void addr_subqueries(adns_query qu, struct timeval now, adns_queryflags qf_extra, const byte *qd_dgram, int qd_dglen)
{
    int i;
    int err;
    int id;
    ...
    for (i = 0; i < addr_nrrtypes; i++)
    {
        ...
        err = adns__mkquery_frdgram(qu->ads,
            &qu->vb,
            &id,
            qd_dgram,
            qd_dglen,
        ...
        err = adns__internal_submit(qu->ads,
            &cqu,
            qu,
            &tinfo_addrsub,
 ...
 *
 */
static adns_status addr_submit(adns_query parent, adns_query *query_r, vbuf *qumsg_vb, int id, unsigned want, adns_queryflags flags, struct timeval now, qcontext *ctx)
{
    /**
     * Isso é efetivamente um substituto para adns__internal_submit,
     * destinado ao caso em que o chamador (possivelmente) deseja
     * apenas um subconjunto dos tipos de registro disponíveis.
     * As regras de gerenciamento de dados e retorno de chamada
     * são as mesmas de adns__internal_submit.
     *
     * Algumas diferenças: a consulta é vinculada à lista de sub
     * processos do processo principal antes de sair (embora o
     * estado do processo principal não seja alterado e não seja
     * vinculado à fila da lista de sub processos); e definimos
     * a parte `tinfo' da estrutura de contexto (sim,
     * modificando *ctx), pois esse é, de fato, o objetivo
     * principal dessa função.
     */

    adns_state ads = parent->ads;
    adns_query qu;
    adns_status err;
    adns_rrtype type = ((adns_r_addr & adns_rrt_reprmask) | (parent->answer->type & ~adns_rrt_reprmask));

    ctx->tinfo.addr.want = want;
    ctx->tinfo.addr.have = 0;
    err = adns__internal_submit(ads,
        &qu,
        parent,
 ...
 *
 */
static adns_status append_addrs(adns_query qu, size_t rrsz, adns_rr_addr **dp, int *dlen, const adns_rr_addr *sp, int slen)
{
    /**
     * Anexe um vetor de registros slen addr, cada um de tamanho rrsz,
     * começando em ap, a um vetor começando em *dp, de comprimento *dlen.
     * Após a conclusão bem-sucedida, *dp e *dlen são atualizados.
     */
...
static void icb_addr(adns_query parent, adns_query child)
{
    ...
    if (!(child->flags & adns__qf_addr_cname) && (parent->flags & adns__qf_addr_answer) && (!!pans->cname != !!cans->cname || (pans->cname && strcmp(pans->cname, cans->cname))))
    {
        /**
         * Detectamos uma inconsistência nos registros CNAME e
         * devemos implantar boas medidas.
         */
        if (!pans->cname)
        {
            /**
             * O sub processo tem um registro CNAME, mas o processo principal não.
             * Devemos descartar todos os endereços dos processos principal e
             * substituir os do sub processos.
             */
        ...
        /**
         * Definimos o CNAME (agora) associado ao processo principal, que
         * já possui registros de endereço apropriados. Construa um datagrama
         * de consulta para esse nome para que possamos emitir consultas em
         * sub processos para as famílias de endereços ausentes. O vbuf do
         * sub processo parece útil para isso.
         */
        err = adns__mkquery(ads,
            &child->vb,
            &id,
            pans->cname,
            strlen(pans->cname),
        ...
        /**
         * Agora cancele os sub processos restantes e tente
         * novamente com o CNAME que escolhemos.
         */
        ...
        if (!(parent->flags & adns_qf_cname_loose))
        {
            qf |= adns_qf_cname_forbid;
        }
    ...
    if ((parent->flags & adns_qf_search) && !pans->cname && cans->status == adns_s_nxdomain)
    {
        /**
         * Estamos procurando uma lista de sufixos e o nome não existe.
         * Usar o do próximo ciclo.
         */
    ...
    {
        adns__query_fail(parent, adns_s_nodata);
    } else
    {
    ...
    x_gtod:
        /**
         * Temos nosso próprio tratamento de falhas, porque adns__must_gettimeofday
         * lida com falhas chamando adns_globalsystemfailure, o que reinseriria a
         * lógica de processamento da consulta.
         */
        adns__diag(ads, -1, parent, "gettimeofday/clock_gettime failed: %s", strerror(errno));
...
static void qs_addr(adns_query qu, struct timeval now)
{
    ...
    {
        qu->ctx.tinfo.addr.want = addr_rrtypes(qu->ads, qu->answer->type, qu->flags);
...
static adns_status pap_domain(const parseinfo *pai, int *cbyte_io, int max, char **domain_r, parsedomain_flags flags)
{
    ...
    st = adns__parse_domain(pai->qu->ads,
        pai->serv,
        pai->qu,
        &pai->qu->vb,
        flags,
 ...
 *
 */
static adns_status pap_findaddrs(const parseinfo *pai, adns_rr_hostaddr *ha, unsigned *want_io, size_t addrsz, int *cbyte_io, int count, int dmstart)
{
    int rri, naddrs;
    unsigned typef;
    unsigned want = *want_io;
    unsigned need = want;
    ...
    for (rri = 0, naddrs = 0; rri < count; rri++)
    {
        ...
        st = pap_addr(pai,
            ...
            addrsz,
            &rdstart,
            rdend, (adns_rr_addr *)(pai->qu->vb.buf + naddrs*addrsz));
    ...
    if (naddrs > 0)
    {
        st = append_addrs(pai->qu,
            addrsz,
            &ha->addrs,
            &ha->naddrs, (const adns_rr_addr *)pai->qu->vb.buf, naddrs);
...
static void icb_hostaddr(adns_query parent, adns_query child)
{
    ...
    st = append_addrs(parent, addrsz, &rrp->addrs, &rrp->naddrs, cans->rrs.addr, cans->nrrs);
    ...
    adns__isort(rrp->addrs, rrp->naddrs, addrsz, parent->vb.buf, div_addr, ads);
...
static adns_status pap_hostaddr(const parseinfo *pai, int *cbyte_io, int max, adns_rr_hostaddr *rrp)
{
    ...
    *cbyte_io = cbyte;
    ...
    st = adns__mkquery_frdgram(pai->ads,
        &pai->qu->vb,
        &id,
        pai->dgram,
        pai->dglen,
    ...
    nflags = adns_qf_quoteok_query | (pai->qu->flags & (adns_qf_want_allaf | adns_qf_ipv6_mapv4));
 ...
 *
 */
static adns_status csp_hostaddr(vbuf *vb, adns_rrtype rrt, const adns_rr_hostaddr *rrp)
{
    const char *errstr;
    adns_status st;
...
static adns_status csp_intofinthost(vbuf *vb, int i)
{
    ...
    {
        /**
         * Atualmente usado apenas para MX cujas prioridades são de 16 bits.
         */
        return adns_s_invaliddata;
    }
 ...
 * _ptr   (ckl,pa +icb_ptr)
 */
static adns_status ckl_ptr(adns_state ads, adns_queryflags flags, union checklabel_state *cls, qcontext *ctx, int labnum, const char *dgram, int labstart, int lablen)
{
    if (lablen)
    {
    ...
    {
        if (!adns__revparse_done(&cls->ptr, dgram, labnum, &ctx->tinfo.ptr.rev_rrtype, &ctx->tinfo.ptr.addr))
        {
            return adns_s_querydomainwrong;
        }
    }
...
static void icb_ptr(adns_query parent, adns_query child)
{
    ...
    {
        if (adns__addrs_equal_raw(&queried->sa, parent->ctx.tinfo.ptr.addr.sa.sa_family,found))
        {
            if (!parent->children.head)
            {
                adns__query_done(parent);
...
static adns_status pa_ptr(const parseinfo *pai, int dmstart, int max, void *datap)
{
    ...
    st = adns__internal_submit(pai->ads,
        &nqu,
        pai->qu,
 ...
 * _srv*  (ckl,(pap),pa*2,mf*2,di,(csp),cs*2,postsort)
 */
static adns_status ckl_srv(adns_state ads, adns_queryflags flags, union checklabel_state *cls, qcontext *ctx, int labnum, const char *dgram, int labstart, int lablen)
{
...
/**
 * pode ser adns_rr_srvraw*.
 */
static adns_status pap_srv_begin(const parseinfo *pai, int *cbyte_io, int max, adns_rr_srvha *rrp)
{
    const byte *dgram = pai->dgram;
    int ti;
    int cbyte;
 ...
 *
 */
static adns_status pa_srvraw(const parseinfo *pai, int cbyte, int max, void *datap)
{
 ...
 *
 */
static adns_status pa_srvha(const parseinfo *pai, int cbyte, int max, void *datap)
{
...
static int di_srv(adns_state ads, const void *datap_a, const void *datap_b)
{
    ...
    /**
     * pode ser const adns_rr_svhostaddr*.
     */
...
static adns_status cs_srvraw(vbuf *vb, adns_rrtype rrt, const void *datap)
{
    ...
    st = csp_srv_begin(vb, (const void*)rrp);
...
static adns_status cs_srvha(vbuf *vb, adns_rrtype rrt, const void *datap)
{
    ...
    st = csp_srv_begin(vb, (const void*)datap);
 ...
 *
 */
static void postsort_srv(adns_state ads, void *array, int nrrs,int rrsz, const struct typeinfo *typei)
{
    /**
     * Tratamos tudo no vetor como se fosse um adns_rr_srvha mesmo
     * que o vetor seja de adns_rr_srvraw. Tudo bem porque eles
     * têm o mesmo prefixo, que é tudo o que acessamos. Também
     * usamos rrsz, em vez de indexação de vetor.
     */
    char *workbegin;
    char *workend;
    char *search;
    ...
    for (workbegin = array, arrayend = workbegin + rrsz * nrrs; workbegin < arrayend; workbegin = workend)
    {
        cpriority = (rr=(void*)workbegin)->priority;

        for (workend = workbegin, totalweight = 0; workend < arrayend && (rr = (void*)workend)->priority == cpriority; workend += rrsz)
        {
            totalweight += rr->weight;
        }

        /**
         * Agora workbegin..(workend-1) incl. são exatamente todos os RRs de
         * cpriority. A partir de agora, workbegin aponta para os registros
         * `restantes': selecionamos um registro por vez (RFC2782 `Regras de
         * uso' e `Formato do SRV RR' subseção `Carga') para colocar em
         * workbegin (trocando com o que foi lá, e então o trabalho
         * avançado começa.
         *
         * workbegin + rrsz < workend; ~> não se preocupe se apenas um.
         */
        for (; workbegin + rrsz < workend; workbegin += rrsz)
        {
            randval = nrand48(ads->rand48xsubi);
            randval %= (totalweight + 1);

            /**
             * Torna em 0..totalweight inclusive; com 2^10 RRs, o peso total
             * deve ser <= 2^26, então a probabilidade de não uniformidade
             * não é pior do que 1 em 2^(31-26), ou seja, 1 em 2^5, ou seja,
             * abs(log(P_intended(RR_i) / P_actual(RR_i)) <= log(2^-5).
             */
            for (search = workbegin, runtotal = 0; (runtotal += (rr = (void*)search)->weight) < randval; search += rrsz)
            {
            }
 ...
 * _opaque (pa, cs)
 */
static adns_status pa_opaque(const parseinfo *pai, int cbyte, int max, void *datap)
{
    adns_rr_byteblock *rrp= datap;
...
static adns_status cs_opaque(vbuf *vb, adns_rrtype rrt, const void *datap)
{
    ...
    for (l = rrp->len, p = rrp->data; l >= 4; l -= 4, p += 4)
    {
...
#define DEEP_TYPE(code, rrt, fmt, memb, parser, comparer, ...) \
    { \
        adns_r_##code&adns_rrt_reprmask, \
        rrt, \
        fmt, \
        TYPESZ_M(memb), \
        mf_##memb, \
        GLUE(cs_, CAR(__VA_ARGS__)), \
        pa_##parser, \
        di_##comparer, \
        adns__ckl_hostname, 0, \
        adns__getrrsz_default, \
        adns__query_send, \
        CDR(__VA_ARGS__) \
    }
...
{
    /**
     * Deve estar em ordem crescente de rrtype !
     *
    ...
    DEEP_TYPE(srv_raw,"SRV",   "raw",     srvraw ,     srvraw,    srv,      srvraw,      .checklabel = ckl_srv, .postsort = postsort_srv),
    FLAT_TYPE(addr,   "A",     "addr",    addr,        addr,      addr,     addr,        .getrrsz = gsz_addr,   .query_send = qs_addr   ),
    DEEP_TYPE(ns,     "NS",    "+addr",   hostaddr,    hostaddr,  hostaddr, hostaddr                                                    ),
    DEEP_TYPE(ptr,    "PTR",   "checked", str,         ptr,       0,        domain,      .checklabel = ckl_ptr                          ),
    DEEP_TYPE(mx,     "MX",    "+addr",   inthostaddr, mx,        mx,       inthostaddr,                                                ),
    DEEP_TYPE(srv,    "SRV",   "+addr",   srvha,       srvha,     srv,      srvha,       .checklabel = ckl_srv, .postsort = postsort_srv),
 ...
 *
 */
static const typeinfo tinfo_addrsub = FLAT_TYPE(none, "<addr>", "sub", addr, addr, 0, addr, .getrrsz = gsz_addr);

/**
 *
 */
static const typeinfo typeinfo_unknown = DEEP_TYPE(unknown, 0, "unknown", byteblock, opaque, 0, opaque);
...
const typeinfo *adns__findtype(adns_rrtype type)
{
    ...
    {
        /**
         * 0x60000000 é reservado para expansão futura.
         */
        return 0;
    }

    if (type & adns_r_unknown)
    {
